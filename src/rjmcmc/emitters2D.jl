...
function calc_area(obs::Observations) 
    min_x = minimum([obs.x - 3 * obs.σ_x for obs in obs.ŷ])
    max_x = maximum([obs.x + 3 * obs.σ_x for obs in obs.ŷ])
    min_y = minimum([obs.y - 3 * obs.σ_y for obs in obs.ŷ])
    max_y = maximum([obs.y + 3 * obs.σ_y for obs in obs.ŷ])
