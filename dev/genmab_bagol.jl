...
# Helper for custom SMLMRender targets (standard report uses render_report)
...
result_smld, diag = run_bagol(smld;
    ...
    sync_interval=500,
    μ_prior_shape=2.0,
    μ_prior_scale=max(est_μ / 2.0, 5.0),  # prior mean ≈ est_μ
...
loc_partition = zeros(Int, length(locs))
for p in partitions
    for idx in p.original_indices
        loc_partition[idx] = p.id
    end
end
...
render_report(locs_smld, bagol_smld;
    output_dir=OUTPUT_DIR, prefix="genmab", pixel_size=2.0, fov=fov)

## ── Partition-colored localizations (SMLMRender) ─────────────────────────────

println("\nPartition-colored localizations...")

# Build one SMLD per partition with distinct colors
n_colors = 20
palette = distinguishable_colors(n_colors, [RGB(1,1,1), RGB(0,0,0)], dropseed=true)

partition_smlds = [SMLMData.BasicSMLD(p.locs, cam, 1, 1) for p in partitions]
partition_cols = [palette[mod1(p.id, n_colors)] for p in partitions]

# Create render target for ROI
partition_target = create_target(roi_xmin, roi_xmax, roi_ymin, roi_ymax; pixel_size=2.0)

# Render partition circles
render(partition_smlds;
    colors = partition_cols,
    strategy = EllipseRender(),
    target = partition_target,
    filename = joinpath(OUTPUT_DIR, "partitions_circles.png")
)
println("  Saved: partitions_circles.png")

# Partitions + BaGoL emitters overlaid (red)
# Use smaller pixel size so ~2nm BaGoL circles are visible
partition_target_fine = create_target(roi_xmin, roi_xmax, roi_ymin, roi_ymax; pixel_size=0.5)
render([partition_smlds..., bagol_smld];
    colors = [partition_cols..., RGB(1.0, 0.0, 0.0)],
    strategy = EllipseRender(),
    target = partition_target_fine,
    filename = joinpath(OUTPUT_DIR, "partitions_with_bagol.png")
)
