# Single N-mer (No Hierarchical)
# ===========================
# Same as nmer_single_test.jl but with μ and shape held fixed at their
# true values (computed from simulation data). Tests spatial assignment
# performance in isolation from hierarchical parameter learning.
#
...
# Output
const OUTPUT_DIR = joinpath(@__DIR__, "output", "nmer_single_nohier")
...
println("="^60)
println("Single $(N_EMITTERS)-mer — Collapsed Gibbs (No Hierarchical)")
println("="^60)
println("\nParameters:")
...
# Compute true blinking statistics from simulation data
TRUE_MU = length(locs) / N_EMITTERS
# Poisson blinking → NegBin shape ≈ ∞; use large finite value
TRUE_SHAPE = 1000.0
println("  No hierarchical — fixed: μ = $(round(TRUE_MU, digits=1)), shape = $(TRUE_SHAPE)")

# =============================================================================
# RUN COLLAPSED GIBBS WITH CALLBACK (FIXED μ/shape)
# =============================================================================

println("\n" * "-"^60)
println("Running collapsed Gibbs sampler (fixed μ=$(round(TRUE_MU, digits=1)), shape=$(TRUE_SHAPE))...")
