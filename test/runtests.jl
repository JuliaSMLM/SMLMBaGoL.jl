...
@testset "SMLMBaGoL.jl Tests" begin
    ...
    @testset "Spatial Utilities" begin
        ...
        loc = SMLMData.Emitter2DFit(0.1, 0.2, 1000.0, 10.0, 0.005, 0.006, 0.0, 0.0, 1, 1, 0, 1)
        ...
        loc2 = SMLMData.Emitter2DFit(0.2, 0.2, 1000.0, 10.0, 0.005, 0.005, 0.0, 0.0, 1, 1, 0, 2)
    ...
    @testset "Partitioning" begin
        ...
        for i in 1:15
            x = 0.5 + randn() * σ
            y = 0.5 + randn() * σ
            push!(locs, SMLMData.Emitter2DFit(x, y, 1000.0, 10.0, σ, σ, 0.0, 0.0, 1, 1, 0, i+20))
        end
    ...
    @testset "Partitioned BaGoL via SMLD" begin
        ...
        for i in 1:5
            push!(locs, SMLMData.Emitter2DFit(0.1 + randn()*σ, 0.1 + randn()*σ, 1000.0, 10.0, σ, σ, 0.0, 0.0, 1, 1, 0, i))
        end
        for i in 1:5
            push!(locs, SMLMData.Emitter2DFit(0.12 + randn()*σ, 0.1 + randn()*σ, 1000.0, 10.0, σ, σ, 0.0, 0.0, 1, 1, 0, i+5))
        end

        # Cluster 2: 1 emitter at (0.5, 0.5)
        for i in 1:5
            push!(locs, SMLMData.Emitter2DFit(0.5 + randn()*σ, 0.5 + randn()*σ, 1000.0, 10.0, σ, σ, 0.0, 0.0, 1, 1, 0, i+10))
        end
