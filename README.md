...
package provides `estimate_se_adjust`, which infers the missing uncertainty `τ` directly
from the data; enable it with `se_adjust=:auto` and BaGoL folds `τ` into each
localization in quadrature (`σ² + τ²`) before grouping.
